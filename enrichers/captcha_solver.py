...
class YunmaProvider(_Provider):
    ...
    def __init__(self, token: str, type_code: str = "900011") -> None:
    ...
    def describe(self) -> str:
        return f"云码(type={self.type_code}, token=...{self.token[-4:] if self.token else '?'})"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--image", default=None, help="用这张本地图片真实调用一次识别")
