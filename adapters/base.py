...
class BaseAdapter:
    ...
    # ------------------------------------------------------------------ #
    # 可选契约：按 URL 区分执行模式（两阶段执行的依据）
    # ------------------------------------------------------------------ #
    def requires_browser_for(self, url: str) -> bool:
        """该 URL 是否必须走浏览器。

        默认沿用类级 `requires_browser`。需要"检索页用浏览器、详情页用 HTTP"这类
        混合策略时覆盖它——只对真正需要 JS 渲染的页面声明 True。

        为什么重要（实测教训）：把本来不需要 JS 的详情页也交给浏览器，单页耗时从
        约 1 秒涨到约 7 秒，还会把内存拉满（Chrome 被系统压死）。legacy 爬虫一直是
        "浏览器只用于列表页 + httpx 抓详情"，迁移时不该丢掉这个优势。
        """
        return bool(self.requires_browser)

    def pop_http_urls(self) -> list[str]:
        """取出"可以用轻量 HTTP 抓取"的 URL（取完即清空）。

        返回非空列表时，引擎会在浏览器阶段结束后自动追加一个 HTTP 阶段来抓这些 URL，
        浏览器只需处理真正的 JS 页面。默认返回空列表 = 全部走同一个执行模式。
        """
        return []
